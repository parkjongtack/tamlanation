        ...
        <table>
			...
			<?php
				...
				*/
				...
				$sql = "select * from as_account where 1=1 and user_type = '1' ".$where." order by ".$array_table_set." ".$array_sort." limit " .(($page-1)*$page_set).",".$page_set;
				$result = mysqli_query($link, $sql);				
				//$count = mysqli_num_rows($result);

				$sql2 = "select * from as_account where 1=1 and user_type = '1' ".$where."";
				...
				<?php } else { ?>
					<?php 
						...
						<tr>
							...
							<td class="delete_box"><a href="javascript:control('<?=$row['user_id']?>');">삭제</a><a href="/as_admin/member_regist_form_general.php?id=<?=$row['user_id']?>&board_type=user_modify" style="background-color: #08AEEA; border:1px solid #0faeea; color: #fff;">수정</a></td>
						</tr>
						<?php $i++; } ?>
					<?php } ?>
            </tbody>
